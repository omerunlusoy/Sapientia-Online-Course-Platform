...
if( $result = $con->query($sql)) {
    ...
    $_SESSION['student_email'] = $row['e_mail'];
    $_SESSION['student_membership'] = $row['membership_type'];
}
else{
...
if(isset($_POST['save_changes_button'])){
    ...
    if( $result = $con->query($sql)) {
        if ($result->num_rows == 0 || $_SESSION['student_email'] == $email){
            $sql = "UPDATE Student SET name = '$name', e_mail = '$email', password = '$password'
                    WHERE SID = '$SID';";
            if( $result = $con->query($sql)) {
...
<head>
    ...
    <link rel="stylesheet" href="Student-Account.css" media="screen">
...
<section class="u-clearfix u-section-1" id="sec-7b6d">
	...
	C6.9,115.6,25.2,125.2,43.2,126.9z"></path></svg>
        ...
        <div class="u-border-11 u-border-palette-4-light-2 u-container-style u-group u-white u-group-1">
            <div class="u-container-layout u-container-layout-1">
                <div class="u-form u-form-1">
                    <form action="#" method="POST" >
                        <div class="u-form-group u-form-name">
                            ...
                            <input type="text" value='<?php echo $_SESSION["student_name"]; ?>' placeholder="Enter your Name" id="name-cba0" name="name" class="u-border-2 u-border-palette-2-light-2 u-input u-input-rectangle u-white" required="">
                        </div>
                        <div class="u-form-email u-form-group">
                            <label for="email-cba0" class="u-label">Email</label>
                            <input type="email" value='<?php echo $_SESSION["student_email"]; ?>' placeholder="Enter a valid email address" id="email-cba0" name="email" class="u-border-2 u-border-palette-2-light-2 u-input u-input-rectangle u-white" required="">
                        </div>
                        <div class="u-form-group u-form-group-3">
                            <label for="text-3d43" class="u-label">Password</label>
                            <input type="password" value='<?php echo $_SESSION["student_password"]; ?>' placeholder="Password" id="text-3d43" name="password" class="u-border-2 u-border-palette-2-light-2 u-input u-input-rectangle u-white">
                        </div>
                ...
                <p class="u-text u-text-default u-text-2">Level : <?php echo $_SESSION["student_membership"]; ?></p>
